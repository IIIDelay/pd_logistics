...
public abstract class DefaultGlobalExceptionHandler {
    ...
    public Result bindException(BindException ex, HttpServletRequest request) {
        ...
        try {
            ...
            if (StrUtil.isNotEmpty(msgs)) {
                return Result.result(ExceptionCode.PARAM_EX.getCode(), StrPool.EMPTY, msgs).setPath(request.getRequestURI());
            }
        } catch (Exception ee) {
        }
    ...
    public Result<String> otherExceptionHandler(Exception ex, HttpServletRequest request) {
        ...
        return Result.result(ExceptionCode.SYSTEM_BUSY.getCode(), StrPool.EMPTY, ExceptionCode.SYSTEM_BUSY.getMsg()).setPath(request.getRequestURI());
    }
