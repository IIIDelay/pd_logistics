<template>
  ...
  >
    ...
    >
      <el-form-item
        :hidden="true"
        :label="$t('table.dictionaryItem.dictionaryId')"
        prop="dictionaryId"
      >
...
export default {
  ...
  methods: {
    ...
    save() {
      const vm = this
      dictionaryItemApi.save(this.dictionaryItem).then(response => {
    ...
    update() {
      dictionaryItemApi.update(this.dictionaryItem).then(response => {
        const res = response.data
        if (res.success) {
