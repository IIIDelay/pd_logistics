...
public class PdCompletableFuture {
    ...
    public static final CompletableFuture<Map> userMapFuture(UserApi api, Set<String> userSet, Integer station, String name, String agencyId) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            /*RespResult<List<User>> result = api.list(userSet.stream().mapToLong(id -> Long.valueOf(id)).boxed().collect(Collectors.toList()), stationId, name, StringUtils.isNotEmpty(agencyId) ? Long.valueOf(agencyId) : null);
...
//        return CompletableFuture.supplyAsync(() -> {
//            RespResult<Org> result = api.get(id);
//            if (result.getIsSuccess()&&result.getData()!=null) {
