...
import org.iiidev.pinda.DTO.TripsTruckDriverDTO;
...
import org.iiidev.pinda.service.ITaskTripsSchedulingService;
...
public class TaskTripsSchedulingServiceImpl implements ITaskTripsSchedulingService {
    ...
    private List<TripsTruckDriverDTO> getTripsTruckDriver(List<TransportTripsTruckDriverDto> transportTripsTruckDriverDtos) {
        ...
        transportTripsTruckDriverDtoMap.forEach((tripsId, transportTripsTruckDriverDtoList) -> {
            ...
            // 获取全部司机id
            Result<List<User>> userResult = userApi.list(transportTripsTruckDriverDtoList.stream().filter(item -> StringUtils.isNotBlank(item.getUserId())).map(item -> Long.parseLong(item.getUserId())).collect(Collectors.toList()), null, null, null);
